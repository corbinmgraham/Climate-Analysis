...
As seen, DP values have quite a few more NA values than DT/DX values. Since are several null-value rows, we will drow rows hat contain null values. Also, when checking precipitation data, we see that it is missing all dates before 1950. This will be removed since the values will be biased towards later dates.
